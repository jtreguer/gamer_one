...
var _twinkle_set: Dictionary = {}  # int -> bool for O(1) lookup
var _twinkle_phase: float = 0.0
var _redraw_accumulator: float = 0.0
var _viewport_size: Vector2 = Vector2.ZERO

const BACKGROUND_REDRAW_INTERVAL: float = 0.05  # 20fps is enough for twinkle


func _ready() -> void:
	_viewport_size = get_viewport_rect().size
	for i in star_count:
		_star_positions.append(Vector2(randf() * _viewport_size.x, randf() * _viewport_size.y))
...
func _process(delta: float) -> void:
	...
	if _redraw_accumulator >= BACKGROUND_REDRAW_INTERVAL:
...
func _draw() -> void:
	# Background fill
	draw_rect(Rect2(Vector2.ZERO, _viewport_size), background_color)

	# Stars
	for i in _star_positions.size():
		var alpha: float = 0.6
		if _twinkle_set.has(i):
